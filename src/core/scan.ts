...
import { globToRegExp, posix, walk } from './util.js';
...
export function isExtractableSourceFile(file: string, config: Config): boolean {
  ...
  if (!rel || path.posix.isAbsolute(rel) || rel.split('/').some((part) => part === '..' || part === '.')) return false;
  if (!TEXT_EXT.has(path.posix.extname(rel))) return false;
  if (NEVER_SCAN.some((pattern) => pattern.test(rel))) return false;
  if (path.posix.extname(rel) === '.html' && config.framework !== 'html') return false;
  const messagesDir = path.posix.normalize(posix(config.messagesDir)).replace(/^\.\/|\/+$/g, '');
  if (rel === messagesDir || rel.startsWith(`${messagesDir}/`)) return false;
  if (config.exclude.some((glob) => globToRegExp(glob).test(rel))) return false;
  if (config.include.length && !config.include.some((glob) => globToRegExp(glob).test(rel))) return false;
  return true;
}

/** Generated output can live outside a conventionally named directory. */
export function isGeneratedSource(content: string): boolean {
  const banner = content.slice(0, 2_000);
  return (
    /@generated/i.test(banner) ||
    /^\s*(?:\/\/+|\/\*+|\*+|#|<!--)\s*[^\n]{0,160}(?:auto(?:matically)?[- ]generated|(?:code )?generated by\b|do not edit[^\n]*(?:generated|codegen)|(?:generated|codegen)[^\n]*do not edit)/im.test(banner)
  );
}
...
export function scanRepo(cwd: string, config: Config): ScanResult {
  ...
  for (const rel of files) {
    if (!isExtractableSourceFile(rel, config)) continue;
    ...
    if (isGeneratedSource(content)) continue;
...
export function findPlaceholders(text: string): string[] {
  return [...new Set(findPlaceholderOccurrences(text))];
}

/** Preserve occurrence counts for integrity checks while scans store a compact unique list. */
export function findPlaceholderOccurrences(text: string): string[] {
